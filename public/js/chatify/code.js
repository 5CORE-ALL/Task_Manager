...
function scrollBottom(container) {
 if ($(container)[0] != "" && $(container)[0] && null && $(container)[0] != "undefined") {
     ...
     var isScrolledToBottom = $(container)[0].scrollHeight - $(container)[0].clientHeight <= $(container)[0].scrollTop + 100;
...
/**
*-------------------------------------------------------------
* Smart scroll function for better UX
*-------------------------------------------------------------
*/
function smartScrollToBottom(container, force = false) {
 if ($(container)[0] != "" && $(container)[0] && null && $(container)[0] != "undefined") {
     // Force scroll (for sending new messages)
     if (force) {
         $(container).stop().animate({
             scrollTop: $(container)[0].scrollHeight
         }, 300);
...
function disableOnLoad(action = true) {
 if (action == true) {
     ...
     // DON'T hide send card - keep it always visible
     ...
     // Always ensure send card is visible
     $('.messenger-sendCard').show().css({'display': 'flex', 'visibility': 'visible', 'opacity': '1'});
 ...
 if ($.trim(msg).length > 0 || hasFile) {
     ...
     $.ajax({
         ...
         beforeSend: () => {
             ...
             // scroll to bottom (force scroll when sending)
         ...
         success: (data) => {
             ...
             if (data.error > 0) {
                 ...
                 // scroll to bottom (force scroll for sent messages)
                 smartScrollToBottom(messagesContainer, true);
 ...
 if (messenger != 0) {
     $.ajax({
         ...
         success: (data) => {
             ...
             // scroll to bottom (force scroll when loading new conversation)
             smartScrollToBottom(messagesContainer, true);
             // remove loading bar
             // NProgress.done();
             // NProgress.remove();
...
channel.bind('messaging', function (data) {
 ...
 if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id) {
     ...
     // Use improved scroll behavior
     if (typeof window.improvedScrollToBottom === 'function') {
         window.improvedScrollToBottom(messagesContainer);
         // Show indicator if user has scrolled up
         if (window.showNewMessageIndicator) {
             window.showNewMessageIndicator();
         }
     } else {
...
channel.bind('client-typing', function (data) {
 ...
 // smart scroll for typing indicator - less aggressive
 if (data.typing == true) {
     smartScrollToBottom(messagesContainer);
 }
});
...
$(document).ready(function() {
    ...
    // Track when user manually scrolls in the message area
    messagesContainer.on('scroll', function() {
        ...
        scrollTimeout = setTimeout(function() {
            var container = messagesContainer[0];
            var isNearBottom = container.scrollHeight - container.clientHeight <= container.scrollTop + 100;
            
            // If user scrolled back to near bottom, reset scroll tracking
            if (isNearBottom) {
    ...
    // Improved scroll to bottom function that respects user behavior
    window.improvedScrollToBottom = function(container, forceScroll = false) {
        if (!container || !container[0]) return;
        
        var containerEl = container[0];
        var isNearBottom = containerEl.scrollHeight - containerEl.clientHeight <= containerEl.scrollTop + 50;
        
        // Force scroll for sent messages or when loading new conversation
        if (forceScroll || (!userHasScrolled && isNearBottom)) {
            container.stop().animate({
    ...
    // Add scroll indicator when there are new messages below
    window.showNewMessageIndicator = function() {
        if (userHasScrolled && !$('.new-message-indicator').length) {
            ...
            $('.new-message-indicator').on('click', function() {
                improvedScrollToBottom(messagesContainer, true);
                $(this).remove();
            });
...
$(document).ready(function() {
    // Force message input to be visible on page load
    setTimeout(function() {
        $('.messenger-sendCard').show().css({
    ...
    setInterval(function() {
        if ($('.messenger-sendCard').length > 0 && $('.messenger-sendCard').is(':hidden')) {
            $('.messenger-sendCard').show().css({
                'display': 'flex !important',
                'visibility': 'visible !important',
                'opacity': '1 !important'
