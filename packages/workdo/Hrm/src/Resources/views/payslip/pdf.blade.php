        ...
        <div class="form-label">
            ...
            <div class="invoice" id="printableArea">
                <div class="invoice-number">
                    <img src="{{asset('images/600x600.png')}}" style="width: 200px;
    height: 200px;
    margin-top: -70px;">
                </div>
                            ...
                            <div class="row text-sm">
                                <div class="col-md-6">
                                    <address>
                                        ...
                                        <strong>{{ __('Employee Id') }} :</strong> 5CORE  {{ $employee->employee_id }}<br>
                                        <strong>{{ __('Department') }} :</strong> {{ $employee->department->name }}<br>
                                ...
                                <div class="col-md-6">
                                    <address>
                                        <strong>{{ __('Designation') }} :</strong> {{ $employee->designation->name }}<br>
                                         <strong>{{ __('Date of Joining') }} :</strong> {{ $employee->company_doj }}<br>
                                        <strong>{{ __('Bank Account No') }} :</strong> {{ $employee->account_number }}<br>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col-lg-4 text-right text-sm">
                                    <div class="invoice-detail-item pb-2 d-flex justify-content-between">
                                        <div class="invoice-detail-name font-weight-bold">{{ __('Current Base') }}
                                        </div>
                                        @php
                                            ...
                                            if ($allowances) {
                                                foreach ($allowances as $allowance) {
                                                    $totalEarning += $allowance->amount;
                                                }
                                            }
                                        @endphp
                                        <div class="invoice-detail-value">
                                            <strong>{{ currency_format_with_sym($totalEarning) }}</strong>
                                        </div>
                                    </div>
                                    @php
                                        // Calculate total earning (basic + allowances)
                                        $totalEarning = $payslip->basic_salary;
                                        $allowances = json_decode($payslipDetail['payslip']->allowance);
                                        if ($allowances) {
                                            foreach ($allowances as $allowance) {
                                        ...
                                        // Get total overtime hours (sum if multiple entries)
                                        $overtimes = json_decode($payslipDetail['payslip']->overtime);
                                        ...
                                        // Get total other payments (sum if multiple entries)
                                        $other_payments = json_decode($payslipDetail['payslip']->other_payment);
                                        $totalOtherPayment = 0;
                                        if ($other_payments) {
                                            foreach ($other_payments as $other_payment) {
                                                $totalOtherPayment += $other_payment->amount;
                                            }
                                        }
                                    
                                        // Calculate payable
                                        $payable = ($totalEarning * $totalOvertimeHours / 200) + $totalOtherPayment;
                                    @endphp
                                     <div class="invoice-detail-item d-flex justify-content-between">
                                        <div class="invoice-detail-name font-weight-bold">{{ __('Payable') }}</div>
                                        <div class="invoice-detail-value">
                                            <strong>{{ currency_format_with_sym($payable) }}</strong>
                                        </div>
                                    </div>
                                    <div class="invoice-detail-item d-flex justify-content-between mt-1">
                                    ...
                                    @php
    $netPayable = $payable - $payslipDetail['totalDeduction'];
@endphp
<div class="invoice-detail-item d-flex justify-content-between">
    <div class="invoice-detail-name font-weight-bold">{{ __('Net Payable') }}</div>
    <div class="invoice-detail-value">
        <strong>{{ currency_format_with_sym($netPayable) }}</strong>
    </div>
</div>
