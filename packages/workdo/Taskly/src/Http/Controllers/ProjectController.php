...
{
    \Log::info('StagingSubmitTask called', $request->all());
    ...
    try {
        ...
        \Log::error('Validation failed in StagingSubmitTask:', $e->errors());
        return response()->json([
            'success' => false,
            'message' => 'Validation failed',
            'errors' => $e->errors()
    ...
    // Sort tasks by start_date ascending (handle null values - put them last)
    $sortedTasks = collect($request->tasks)->sortBy(function($task) {
        return $task['start_date'] ?? '9999-12-31 23:59:59'; // Put null dates at the end
    })->values()->all();
    ...
    foreach ($sortedTasks as $index => $task) {
        ...
        // Only check for duplicates created in the last 5 minutes to allow adding new tasks to existing events
        $existingTask = StagingTask::where('event_id', $request->event_id)
            ->where('task', $task['task'] ?? '')
            ->where('group', $task['group'] ?? '')
            ->where('assignee_id', $task['assignee_id'] ?? null)
            ->where('start_date', $task['start_date'] ?? null)
            ->where('end_date', $task['end_date'] ?? null)
            ->where('created_at', '>=', now()->subMinutes(5)) // Check for duplicates created in last 5 minutes
            ->first();
        
        if ($existingTask) {
            \Log::warning('Duplicate task detected (created in last 5 minutes), skipping: ' . ($task['task'] ?? ''));
            $createdTasks[] = $existingTask;
            continue;
        }
...
private function assignStagingTaskToBoard($stagingTask, $workspace = null)
{
    \Log::info('assignStagingTaskToBoard called for staging task: ' . $stagingTask->id);
    
    $currentWorkspace = $workspace ?: getActiveWorkSpace();
    
    // Get the first stage
    $stage = Stage::where('workspace_id', '=', $currentWorkspace)->orderBy('order')->first();
    if (!$stage) {
        \Log::error('No stage found for workspace: ' . $currentWorkspace);
        return;
    }
    \Log::info('Using stage: ' . $stage->id . ' - ' . $stage->name);

    // Get assignee email
    $assignee = User::find($stagingTask->assignee_id);
    if (!$assignee) {
        \Log::error('Assignee not found: ' . $stagingTask->assignee_id);
        return;
    }
    \Log::info('Assignee: ' . $assignee->email);
    ...
    $taskData = [
        ...
        'assign_to' => $assignee->email,
        'assignor' => $stagingTask->assignor_id ? User::find($stagingTask->assignor_id)->email : $objUser->email,
        'status' => $stagingTask->status, // Use the staging task's status (which is now the stage name)
        'workspace' => $currentWorkspace,
        'is_data_from' => 'staging_task_' . $stagingTask->id, // Link to staging task
        'eta_time' => $stagingTask->eta_time ?? 0, // Add ETC time
    ...
    \Log::info('Created task: ' . $task->id . ' with status: ' . $task->status . ' assigned to: ' . $task->assign_to);

    // Log activity
    $this->logTaskCreation($task->title, 'Task created from staging event and assigned to: ' . $assignee->email);
...
private function assignNextStagingTask($completedTask, $workspace = null)
{
    \Log::info('assignNextStagingTask called for completed task: ' . $completedTask->id);
    
    $currentWorkspace = $workspace ?: getActiveWorkSpace();
    
    // Find the next task in the event that hasn't been assigned yet
    // Look for tasks that are still 'pending' (not yet assigned to board)
    $nextTask = StagingTask::where('event_id', $completedTask->event_id)
        ->where('status', 'pending')
        ->orderBy('start_date')
        ->first();

    \Log::info('Next task found: ' . ($nextTask ? $nextTask->id : 'none'));
    
    if ($nextTask) {
        $firstStage = Stage::where('workspace_id', '=', $currentWorkspace)->orderBy('order')->first();
        $firstStageName = $firstStage ? $firstStage->name : 'Todo';
        
        $nextTask->update(['status' => $firstStageName]);
        $this->assignStagingTaskToBoard($nextTask, $currentWorkspace);
        \Log::info('Next staging task ' . $nextTask->id . ' assigned with status: ' . $firstStageName);
    }
}

public function StagingGetTasks($eventId)
{
    \Log::info('StagingGetTasks called for event_id: ' . $eventId);
    
    $tasks = StagingTask::where('event_id', $eventId)
        ->with(['assignee', 'assignor'])
        ->orderBy('start_date')
        ->get()
        ->map(function ($task) {
            // Format duration string for display (same format as task board)
            $duration = '';
            if ($task->start_date && $task->end_date) {
                try {
                    $start = \Carbon\Carbon::parse($task->start_date);
                    $end = \Carbon\Carbon::parse($task->end_date);
                    $duration = $start->format('MMM d, Y h:i A') . ' - ' . $end->format('MMM d, Y h:i A');
                } catch (\Exception $e) {
            ...
            return [
                ...
                'group' => $task->group,
                'task' => $task->task,
                'assignor' => $task->assignor ? $task->assignor->name : '',
    ...
    public function updateEtcDone(Request $request)
{
    ...
    public function taskUpdate(Request $request, $taskID)
    {
        ...
        $currentWorkspace = getActiveWorkSpace();
            ...
            // Check if this is a staging task and status changed to completed
            if (str_starts_with($task->is_data_from ?? '', 'staging_task_')) {
                $stagingTaskId = str_replace('staging_task_', '', $task->is_data_from);
                $stagingTask = \App\Models\StagingTask::find($stagingTaskId);
                if ($stagingTask) {
                    // Update staging task status to match
                    $stagingTask->update(['status' => $post['status']]);
                    \Log::info('Updated staging task ' . $stagingTaskId . ' status to ' . $post['status']);
                }
            }
    ...
    public function taskStatusUpdate(Request $request, $projectID, $taskID)
    {
        $currentWorkspace = getActiveWorkSpace();
        ...
        if ($old_status != $new_status) {
            $new_stage = Stage::where('workspace_id', '=', $currentWorkspace)->where('name', $new_status)->first();
            $old_stage = Stage::where('workspace_id', '=', $currentWorkspace)->where('name', $old_status)->first();
            ...
            if ($new_stage) {
                ...
                ActivityLog::create([
                    ...
                    'remark' => json_encode([
                        'title' => $task->title,
                        'old_status' => $old_stage ? $old_stage->name : $old_status,
...
{
    \Log::info('=== TASK DESTROY STARTED === taskID: ' . $taskID);
    ...
    if ($task) {
        \Log::info('Task destroy: task ID ' . $task->id . ', is_data_from: ' . ($task->is_data_from ?? 'null'));
        if (str_starts_with($task->is_data_from ?? '', 'staging_task_')) {
            $currentWorkspace = $task->workspace; // Use task's workspace
            $stagingTaskId = str_replace('staging_task_', '', $task->is_data_from);
            $stagingTask = \App\Models\StagingTask::find($stagingTaskId);
            if ($stagingTask) {
                // Update the staging task status to match the deleted task's status
                $stagingTask->update(['status' => $task->status]);
                // Always assign the next task when a staging task is deleted
                $this->assignNextStagingTask($stagingTask, $currentWorkspace);
                \Log::info('Staging task ' . $stagingTaskId . ' status updated to ' . $task->status . ', next task assigned');
            } else {
                \Log::error('Staging task not found: ' . $stagingTaskId);
            }
        } else {
            \Log::info('Task is not a staging task');
        }
    }
    ...
    public function TaskList(ProjectTaskDatatable $dataTable)
    {
        if (\Auth::user()->isAbleTo('task manage')) {
            ...
            // Build base query with conditions applied directly
            $competeTaskQuery = Task::where('status','Done')
                ->where(function($q) {
                    $q->where('is_missed', 0)
            ...
            $pendingTaskQuery = Task::whereNotIn('status',['Done'])
                ->where(function($q) {
                    $q->where('is_missed', 0)
                      ->orWhere(function($sub) {
                          $sub->where('is_automate_task', 0);
            ...
            // Non-team leaders: count tasks where they are assignee OR assignor
                ...
                // Explicitly apply filter for non-team leaders
                $competeTaskQuery->where(function ($q) use ($objUser) {
                    $q->whereRaw("FIND_IN_SET(?, assign_to)", [$objUser->email])
                      ->orWhere('assignor', $objUser->email);
                });
     ...
     public function taskCountData(Request $request)
    {
            ...
            // Only president@5core.com can view all tasks (override all existing visibility)
            if($isPresident)
            {
