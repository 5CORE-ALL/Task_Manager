...

{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $rowcolumn = ['title', 'priority', 'status', 'assign_to', 'assigner_name', 'start_date', 'end_date', 'group', 'all_links','links','link_3','link_4','link_5','link_6','link_7','link_8','link_9','created_at', 'checkbox', 'completion_day','imagefile', 'flag_raise'];
...
if ($toggleFilter === 'overdue') {
    ...
    $task->whereExists(function($query) {
        $query->select(DB::raw(1))
            ...
            ->whereExists(function($subQuery) {
                $subQuery->select(DB::raw(1))
                    ...
                    ->whereRaw("FIND_IN_SET(assignee_users.email, tasks.assign_to) > 0");
            })
            // AND flag description contains task title (flexible matching)
            ->where(function($q) {
                $q->whereRaw("flag_raises.description LIKE CONCAT('%', tasks.title, '%')")
                  ->orWhereRaw("flag_raises.description LIKE CONCAT('Task: ', tasks.title, '%')");
            });
    });
    ...
    $task->where(function($query) use ($currentDate) {
        $query->where('tasks.is_missed', 1)
              ->orWhere(function($subQuery) use ($currentDate) {
                  $subQuery->where('tasks.due_date', '<', $currentDate)
