    ...
    function generateSubMenu($menuItems, $parent = null)
    {
        ...
        foreach ($filteredItems as $item) {
            ...
            $html .= '<a href="' . (!empty($item['route']) ? route($item['route']) : '#!') . '" class="dash-link"';
              if (($item['parent'] == null) && !empty($item['route'])) {
                    $html .= 'target="_blank"  >';
              }elseif(!empty($item['route']) && ($item['route']=="redirect.Recruitment" || $item['route']=="redirect.resources" || $item['route']=="redirect.5policy" || $item['route']=="redirect.policy" || $item['route']=="redirect.Training" || $item['route']=="redirect.link" || $item['route']=="salary.incentive" || $item['route']=="salary.increment" || $item['route']=="salary.incentive-records" || $item['route']=="salary.increment-records" || $item['route']=="salary.board" || $item['route']=="projecttask.automate.list" || $item['route']=="projecttask.done.list"  || $item['route']=="projecttask.track" || $item['route']=="report.view"  || $item['route']=="projecttask.list" || $item['route']=="redirect.7-Day-Onboarding" || $item['route']=="redirect.30-Day-Onboarding" || $item['route']=="redirect.improvement-request" || $item['route']=="redirect.Monthly-Growth" || $item['route']=="redirect.Shipping" || $item['route']=="redirect.form") ){
                   $html .= 'target="_blank"  >';
              } else
              {
...
// Format name as First Name + Last Initial (e.g., "Rashmi Bhavsar" -> "Rashmi B")
if (!function_exists('formatUserName')) {
    function formatUserName($name)
    {
        ...
        // If there's a last name, get its first character
