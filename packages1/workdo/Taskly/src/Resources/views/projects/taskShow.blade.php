...
<div class="modal-body">
    <div class="p-2">
        <div class="form-control-label fw-bold">{{ __('Description')}}:</div>
        ...
        <div class="row mb-4">
            <!--<div class="col-md-3">-->
            <!--    <div class="form-control-label">{{ __('Group')}}</div>-->
            <!--    <p class="mt-1">{{ ($task->Group) }}</p>-->
            <!--</div>-->
            <div class="col-md-3">
                <div class="form-control-label fw-bold">{{ __('Group') }}</div>
                <p class="mt-1">
                    {{ isset($task->group) ? $task->group : __('Not Assigned') }}
                </p>
            </div>
            ...
            <div class="col-md-3">
             ...
             <div class="col-md-3">
                ...
                @if($users = $task->users())
                    @foreach($users as $user)
                        <div class="d-flex align-items-center">
                            <img alt="image" data-bs-toggle="tooltip" data-bs-placement="top" title="{{$user->name}}" 
                                 @if($user->avatar) src="{{get_file($user->avatar)}}" 
                                 @else src="{{ get_file('avatar.png')}}"  
                                 @endif class="rounded-circle me-2" width="20px" height="20px">
                            <span>{{ $user->name }}</span>
                        </div>
                    @endforeach
                @endif
              ...
              @if($task->link1)
            <div class="col-md-12">
                <div class="form-control-label fw-bold">{{ __('L1')}}</div>
                <p class="mt-1">{{ ($task->link1) }}</p>
            </div>
            @endif
            @if($task->link2)
             <div class="col-md-12">
                <div class="form-control-label fw-bold">{{ __('L2')}}</div>
                <p class="mt-1">{{ ($task->link2) }}</p>
            </div>
            @endif
        ...
        <div class="row mb-4">
                  <div class="col-md-12">
                        <div class="form-control-label fw-bold">{{ __('Task Title') }}</div>
    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
        <li class="nav-item">
            <a class=" nav-link active" id="comments-tab" data-bs-toggle="tab" href="#comments-data" role="tab" aria-controls="home" aria-selected="false"> {{ __('Notes') }} </a>
        </li>
    ...
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade active show" id="comments-data" role="tabpanel" aria-labelledby="home-tab">
            ...
            <form method="post" id="form-comment" data-action="{{route('comment.store',[$task->id,$clientID])}}">
                ...
                <li class="row media border-bottom mb-3">
                        ...
                        @auth('web')
                            <div class="col-auto text-end row_line_style">
                                ...
                                <a href="#" class="action-btn btn-danger mt-1 btn btn-sm d-inline-flex align-items-center delete-comment"  data-toggle="tooltip" title="{{__('Delete')}}" data-url="{{route('comment.destroy',[$task->id,$comment->id])}}">
        ...
        <div class="tab-pane fade  show" id="members-data" role="tabpanel" aria-labelledby="home-tab">
            <div class="form-group col-md-12">
                ...
                {{ Form::model($task, array('route' => array('tasks.members',[$task->id]), 'method' => 'Post')) }}
        ...
        <div class="tab-pane fade" id="file-data" role="tabpanel" aria-labelledby="profile-tab">
            ...
            <div class="form-group m-0">
                <form method="post" id="form-file" enctype="multipart/form-data" data-url="{{ route('comment.store.file',[$task->id,$clientID]) }}">
                     @csrf
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-auto">
                                    ...
                                    @auth('web')
                                        ...
                                        <a href="#" class="action-btn btn-danger  btn btn-sm d-inline-flex align-items-center delete-comment-file" data-toggle="tooltip" title="{{__('Delete')}}" data-url="{{route('comment.destroy.file',[$task->id,$file->id])}}">
                                            <i class="ti ti-trash"></i>
                                        </a>
                                        @endpermission
                                    @endauth
        ...
        @permission('sub-task create')
            ...
            <form method="post" class="collapse" id="form-subtask" data-action="{{route('subtask.store',[$task->id,$clientID])}}">
                @csrf
                <div class="row">
                    <div class="col-6">
                ...
                @foreach($task->sub_tasks as $subTask)
                    <li class="list-group-item row ">
                        <div class="form-check form-switch d-inline-block d-flex justify-content-between">
                            <div>
                                <input type="checkbox" class="form-check-input" name="option" id="option{{ $subTask->id }}" @if($subTask->status) checked @endif data-url="{{route('subtask.update',[$subTask->id])}}">
                                <label class="custom-control-label form-control-label" for="option{{ $subTask->id }}">{{$subTask->name}}</label>
                            </div>
                            <div>

                                @permission('sub-task delete')
                                <a href="#" class="action-btn btn-danger  btn btn-sm d-inline-flex align-items-center delete-subtask" data-toggle="tooltip" title="{{__('Delete')}}" data-url="{{route('subtask.destroy',[$subTask->id])}}">
                                    <i class="ti ti-trash"></i>
                                </a>
                                @endpermission
                            </div>
