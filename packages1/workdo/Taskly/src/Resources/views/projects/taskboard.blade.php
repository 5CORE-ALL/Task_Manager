...
@section('page-action')
    <div class="d-flex">
        ...
        @endpermission
        <button type="button" class="btn btn-sm btn-success me-2 change-assignee-btn" id="change-assignee-btn" disabled data-bs-toggle="tooltip" data-bs-original-title="{{ __('Change Assignee') }}">
                                    ...
                                    @foreach ($stage->tasks as $task)
                                        <div class="card" id="{{ $task->id }}">
                                            <div class="position-absolute top-0 start-0 pt-3 ps-3">
                                                <input type="checkbox" class="task-checkbox form-check-input" value="{{ $task->id }}" style="margin-right: 5px;">
...
<!-- Bulk Assignee Update Modal -->
<div class="modal fade" id="bulk-assignee-modal" tabindex="-1" aria-labelledby="bulkAssigneeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title" id="bulkAssigneeModalLabel">
        ...
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          ...
          <div class="mb-3">
                    <label for="bulk-assignee-select" class="form-label fw-bold">Select New Assignee</label>
                    <select class="form-select" id="bulk-assignee-select" required>
              ...
              @endif
                    ...
                    <div class="form-text">This will update the assignee for all selected tasks.</div>
                </div>
                <div id="bulk-assignee-selected-count" class="alert alert-info mb-0" style="display: none;">
                    <i class="fas fa-info-circle me-2"></i>
                    <span id="selected-tasks-count">0</span> task(s) selected
          </div>
      </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" id="bulk-assignee-update-btn">
                    <i class="fas fa-save me-1"></i>Update Assignee
        </button>
      </div>
    </div>
  </div>
...
@if ($project && $currentWorkspace)
    @push('scripts')
        ...
        <script>
            ...
            // Update assignee button state based on task selection
            $(document).on("click", ".task-checkbox", function() {
                ...
                if (selectedIds.length > 0) {
                    $('#change-assignee-btn').prop('disabled', false);
                } else {
                    $('#change-assignee-btn').prop('disabled', true);
                }
            });

            // Open bulk assignee modal
            $('#change-assignee-btn').on('click', function() {
                let selectedIds = $(".task-checkbox:checked").map(function() {
                    return this.value;
                }).get();
                ...
                if (selectedIds.length === 0) {
                    if (typeof toastrs === 'function') {
                        toastrs('Warning', 'Please select at least one task', 'warning');
                } else {
                ...
                // Show selected count
                $('#selected-tasks-count').text(selectedIds.length);
                $('#bulk-assignee-selected-count').show();
                
                // Reset form and show modal
                $('#bulk-assignee-select').val('');
                $('#bulk-assignee-modal').modal('show');
            });

            // Handle assignee update
            $('#bulk-assignee-update-btn').on('click', function() {
                let selectedIds = $(".task-checkbox:checked").map(function() {
                    return this.value;
                }).get();
                
                let assigneeEmail = $('#bulk-assignee-select').val();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.is_success) {
                            ...
                            // Close modal
                            $('#bulk-assignee-modal').modal('hide');
                    ...
                    error: function(xhr, status, error) {
                        console.error('Assignee update error:', xhr, status, error);
            ...
            $('#bulk-assignee-modal').on('hidden.bs.modal', function() {
                $('#bulk-assignee-select').val('');
                $('#bulk-assignee-selected-count').hide();
                $('#bulk-assignee-update-btn').prop('disabled', false).html('<i class="fas fa-save me-1"></i>Update Assignee');
            });
