...
Route::group(['middleware' => ['web', 'auth', 'verified','PlanModuleCheck:Hrm']], function () {
    ...
    return redirect()->away('https://training.5coremanagement.com/admin/login');
})->name('redirect.Training');

    Route::get('/redirect/policy', function () {
    return redirect()->away('https://www.5core.com/pages/5-core-leave-policy');
})->name('redirect.policy');

    Route::get('/redirect/7-Day-Onboarding', function () {
    return redirect()->away('https://docs.google.com/forms/d/e/1FAIpQLSfqJjU9SHvNWAsMrk0SSKFd_INseq1HGjfNZFmJjCC0IwqSzw/viewform');
})->name('redirect.7-Day-Onboarding');

    Route::get('/redirect/30-Day-Onboarding', function () {
    return redirect()->away('https://docs.google.com/forms/d/e/1FAIpQLSfryXGWON2qLOwjA_6U6G6WiUWcaASDsdb4AMKks0HhNNCKBw/viewform');
})->name('redirect.30-Day-Onboarding');

    Route::get('/redirect/improvement-request', function () {
    return redirect()->away('https://docs.google.com/forms/d/e/1FAIpQLSf0omRGcPejmbKE9N0pf3fZiq_Jn26l0wJwKUN88qM4Y20Ykg/viewform');
})->name('redirect.improvement-request');


    Route::get('/redirect/Monthly-Growth', function () {
    return redirect()->away('https://docs.google.com/forms/d/e/1FAIpQLSdc4C1GeC5iuhPvPYUh8-7si6W0wKXnJYbNxfxoZJfQ7VVlqw/viewform');
})->name('redirect.Monthly-Growth');
    ...
    return redirect()->away('https://www.5core.com/pages/5-core-policy');
})->name('redirect.5policy');

    Route::get('/redirect/onboarding', function () {
    return redirect()->away('http://onboarding.5coremanagement.com/');
})->name('redirect.onboarding');


    Route::get('/redirect/form', function () {
    return redirect()->away('https://new-form.5coremanagement.com/');
})->name('redirect.form');
